...
def scrap(host, max_depth = 3):
    ...
    while queue:
        ...
        imagePath = image_path + str(node['id']) + '.png'
...
def readTrees(path='./trees/'):
    files = [f for f in listdir(path) if isfile(join(path, f))]
    trees = []
    for file in files:
        fd = open(join(path, file), 'r')
...
#sim_mat = compareTrees(readTrees())
#pickle.dump(sim_mat, open('./sim.p', 'wb'))

#sim_mat = pickle.load(open('./sim.p', 'rb'))
#clusters = clustering_DBSCAN(sim_mat)
#pickle.dump(clusters, open('./clusters.p', 'wb'))

#clusters = pickle.load(open('./clusters.p', 'rb'))
#decodeClusters(clusters, data)
